...
*/
let Datafeeds = {};
const chartData = require("./data/data");
Datafeeds.UDFCompatibleDatafeed = function(datafeedURL, updateFrequency, toCoinName) {
...
Datafeeds.UDFCompatibleDatafeed.prototype._fireEvent = function(event,
                                                                argument) {
	if (this._callbacks.hasOwnProperty(event)) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getMarks = function(symbolInfo,
                                                              rangeStart,
                                                              rangeEnd,
                                                              onDataCallback,
                                                              resolution) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getTimescaleMarks = function(symbolInfo,
                                                                       rangeStart,
                                                                       rangeEnd,
                                                                       onDataCallback,
                                                                       resolution) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.searchSymbols = function(searchString,
                                                                   exchange,
                                                                   type,
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
	...
	if (!this._configuration.supports_group_request) {
		let scale = new Array(that._toCoinName.accuracy);
		let scaleNum = "1";
		for (let i = 0; i < scale.length; i++) {
			scaleNum += "0";
		}
		let data = {
			"name": that._toCoinName.toCoin,
			...
			"has_no_volume": false,
			"description": that._toCoinName.toCoin,
			"type": "stock",
			"supported_resolutions": ["1", "5", "15", "30", "60", "240", "1D"],
			"pricescale": parseInt(scaleNum),
			"ticker": that._toCoinName.toCoin
		};
		onResultReady(data);
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo,
                                                             resolution,
                                                             rangeStartDate,
                                                             rangeEndDate,
                                                             onDataCallback,
	...
	if (bars.length > 0) {
		if (rangeStartDate < bars[0].time / 1000 && window.sessionStorage.time_id === "0") {
			onDataCallback([], {
				noData: true
			});
			window.sessionStorage.setItem("time_id", "1");
			return false;
		}
	}
	// this._send(this._datafeedURL + '/index/history/klined', {
	//     symbol: this._toCoinName.typeName + '-' + symbolInfo.ticker.toUpperCase(),
	//     resolution: this.getStepByResolution(resolution)
	// }).done(function (response) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.subscribeBars = function(symbolInfo,
                                                                   resolution,
                                                                   onRealtimeCallback,
                                                                   listenerGUID,
...
Datafeeds.UDFCompatibleDatafeed.prototype.calculateHistoryDepth = function(period,
                                                                           resolutionBack,
                                                                           intervalBack) {
	if (period === "D") {
		return {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getQuotes = function(symbols,
                                                               onDataCallback,
                                                               onErrorCallback) {
	//  取消了商品报价模块 此方法不会调用
	this._send(this._datafeedURL + "/quotes", { symbols: symbols }).done(function(response) {
		var data = JSON.parse(response);
		if (data.s === "ok") {
			//	JSON format is {s: "status", [{s: "symbol_status", n: "symbol_name", v: {"field1": "value1", "field2":
...
Datafeeds.UDFCompatibleDatafeed.prototype.subscribeQuotes = function(symbols,
                                                                     fastSymbols,
                                                                     onRealtimeCallback,
                                                                     listenerGUID) {
...
Datafeeds.SymbolsStorage.prototype._onExchangeDataReceived = function(exchangeName,
                                                                      data) {
...
Datafeeds.SymbolSearchComponent.prototype.searchSymbols = function(searchArgument,
                                                                   maxSearchResults) {
	...
	searchArgument.onResultReadyCallback(
		results.sort(function(weightedItem1, weightedItem2) {
...
Datafeeds.DataPulseUpdater = function(datafeed, updateFrequency) {
				...
				that._datafeed.getBars(
					...
					function(bars) {
						...
						var isNewBar =
							    !isNaN(_subscriptionRecord.lastBarTime) &&
							    lastBar.time > _subscriptionRecord.lastBarTime;
						//	Pulse updating may miss some trades data (ie, if pulse period = 10 secods and new bar is started 5
						// seconds later after the last update, the old bar's last 5 seconds trades will be lost). Thus, at fist we
						// should broadcast old bar updates when it's ready.
						if (isNewBar) {
							if (bars.length < 2) {
								throw new Error(
	...
	if (typeof updateFrequency != "undefined" && updateFrequency > 0) {
		var intervalId = setInterval(update, updateFrequency);
...
Datafeeds.DataPulseUpdater.prototype.subscribeDataListener = function(symbolInfo,
                                                                      resolution,
                                                                      newDataCallback,
                                                                      listenerGUID) {
...
Datafeeds.DataPulseUpdater.prototype.periodLengthSeconds = function(resolution,
...
Datafeeds.QuotesPulseUpdater.prototype.subscribeDataListener = function(symbols,
                                                                        fastSymbols,
                                                                        newDataCallback,
                                                                        listenerGUID) {
	if (!this._subscribers.hasOwnProperty(listenerGUID)) {
